...
using PowerBI_API_NetCore_Sample;

namespace Embed_Sample.Pages
{
    public class GetReportModel : PageModel
    {
        ...
        public GetReportModel(IConfiguration configuration)
        {
        ...
        public void OnGet(string id)
        {
            if (Request.Query.ContainsKey("code"))
            {
                if (_accessToken == null)
                {
                    ...
                    Task.Run(async () => await EmbedReport(id)).Wait();
                }
                else
                {
        ...
        private async Task EmbedReport(string reportId)
        {
            using (var client = new PowerBIClient(new Uri(AppSettings.ApiUrl), new TokenCredentials(AccessToken, "Bearer")))
            {
                ...
                else
                {
                    ...
                    string message = "No report available in workspace with ID " + reportId + ", Please fill a group id with existing report in appsettings.json file";
